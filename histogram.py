...
print('%d runs with size %d, stride %d -> median throughput = %.1f MB/s' %
      (runcount, size, stride, np.median(data)))
print('min=%.1f, max=%.1f, med=%.1f, avg=%.1f' %
      (min(data), max(data), np.median(data), np.average(data)))
